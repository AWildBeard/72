...
release:
	$(eval LDFLAGS=-w -s -extldflags "-static" -X main.buildType=release)
	$(eval GCFLAGS=)
	@:
...
build: clean
	...
	GOARM=${GOARM} GOARCH=${GOARCH} GOOS=${GOOS} ${BUILD} -p $(shell nproc) -ldflags="${LDFLAGS}" -gcflags="${GCFLAGS}" -o ${OUT}
